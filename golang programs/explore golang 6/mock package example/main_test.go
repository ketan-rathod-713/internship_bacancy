...
func (m *mockRand) randomInt(max int) int {
	args := m.Called(max) // it keeps record that it was called with the value that passed in for max // it also returns the Arguments object which contains the return values that we have specified in the test to returned if the function gets the given value of max. NOTE : here max is a variable.
	return args.Int(0)    // here we are telling that return the zeroth value only.
}
